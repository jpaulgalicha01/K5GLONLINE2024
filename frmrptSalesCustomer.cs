...
{
    public partial class frmrptSalesCustomer : Form
    {
        ...
        public frmrptSalesCustomer()
        {
            ...
            { 
             ...
             new { Text = "Sales By Customer Detailed", Value = "02" }, 
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            else if (cbortprint.SelectedValue.ToString() == "02")
            {
                ...
                {
                    MessageBox.Show("Please enter Customer", "GL");
                    cboPC.Focus();
                }
                else if (Convert.ToDateTime(txtBeginDate.Text) > Convert.ToDateTime(txtEndDate.Text))
                {
        ...
        {
            string sqlstatement;
            sqlstatement = "SELECT Reference, Category, TDate, StockNumber, Item, ClassDesc, ConvertedQty, Piece, IB, TotSales, TotVAT, TotDisct, PClassDesc ";
            sqlstatement +="FROM  ViewGP WHERE TDate BETWEEN '" + txtBeginDate.Text + "' And  '" + txtEndDate.Text + "' AND ControlNo ='" + cboControlNo.SelectedValue.ToString() + "' ORDER BY TDate DESC, Item";
            ClsGetConnection1.ClsGetConMSSQL();
            myconnection = new SqlConnection(ClsGetConnection1.plsMyConMSSQL);
            myconnection.Open();
            mycommand = myconnection.CreateCommand();
            ...
            CRSalesCustomerDetailed objRpt = new CRSalesCustomerDetailed();
            ...
            TextObject varTextSalesman = (TextObject)objRpt.ReportDefinition.Sections["Section1"].ReportObjects["TextCustomer"];
            varTextSalesman.Text = cboControlNo.Text;
